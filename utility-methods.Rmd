...
We estimated trips for three purposes - shopping, social (visiting friends and family), and leisure.
These are combined on https://nptscot.github.io under the joint category of 'Other Everyday' trips.

We used data from the England's National Travel Survey 2019 to estimate the total number of trips undertaken per person per day, as AADT. 
This gives an average of 953 trips per person per year, which equates to 2.61 trips per person per day. 
Equivalent data from Scotland could not be identified to do this.
<!-- talk about scotland england comparisons -->

The number of daily trips for each purpose were determined using the trip purpose percentage breakdown from the Scottish Household Survey 2012-2019 (after adjusting these percentages to remove the 'Go Home' category).
This resulted in 25.1% of trips being assigned for shopping, 11.7% for social, and 6.3% for leisure.

## Mode share

The mode shares were estimated using the mean mode shares from the Scottish Household Survey travel diaries in table 16 of Transport and Travel in Scotland 2019 Local Authority Tables.
...
For shopping, we assumed that the cycling mode share was half of this, with the mode share for the other modes (car, public transport, walking and taxi/other) increased accordingly. 
<!-- This corresponds to data from xxxx that shows lower cycle mode share for shopping trips. -->
...
This includes gyms, swimming pools, nightclubs, cinemas, etc. 
These POIs were assigned to the nearest point on a 500m grid, giving a density measure.
...
Where the trip origins or destinations were assigned based on Intermediate Zone populations, we disaggregate these desire lines and assign them to randomised points on the road network of the zone. 
...
Finally, the desire lines are routed on the road network, using the CycleStreets routing algorithms for fast, balanced and quiet routes.
