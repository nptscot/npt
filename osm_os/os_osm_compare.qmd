...
# clean coherent network
```{r}
# load edingburgh as example
cn_edinburgh = sf::st_read("outputdata/2024-09-01/edinburgh_and_lothians/coherent_networks/city_of_edinburgh_2024-09-01_3_coherent_network.geojson") |> st_transform(crs = "EPSG:27700")

# create a buffer around the network
cn_edinburgh_buffer = sf::st_buffer(cn_edinburgh, dist = 1)

#find os road within the buffer

os_edinburgh = os[sf::st_union(cn_edinburgh_buffer), , op = sf::st_within]

cn_edinburgh_buffer = sf::st_buffer(cn_edinburgh, dist = 20)
combined_net_edinburgh = combined_net[sf::st_union(cn_edinburgh_buffer), , op = sf::st_within]

os_edinburgh = corenet::removeDangles(os_edinburgh,6)

os_edinburgh_NPT = stplanr::rnet_merge(os_edinburgh, combined_net_edinburgh, , max_angle_diff = 10, dist = 15, funs = list(all_fastest_bicycle_go_dutch = mean))     

# Convert geometry type from MULTILINESTRING to LINESTRING without merging lines
os_edinburgh_NPT <- st_cast(os_edinburgh_NPT, "LINESTRING")

# Calculate average all_fastest_bicycle_go_dutch for each name_1 without merging lines
os_edinburgh_NPT_group <- os_edinburgh_NPT |>
  group_by(name_1) |>
  summarize(
...
dim(os_edinburgh_NPT_group)
st_write(os_edinburgh_NPT_group, "osm_os/os_edinburgh_NPT_group_1.geojson", delete_dsn = TRUE)
...
cn = sf::st_read("outputdata/2024-09-01/edinburgh_and_lothians/coherent_networks/city_of_edinburgh_2024-09-01_2_coherent_network.geojson") 
# cn = sf::st_cast(cn, "LINESTRING")
geometry_type = sf::st_geometry_type(cn)
print(unique(geometry_type))
mapview::mapview(cn)
st_write(cn, "osm_os/cn.geojson", delete_dsn = TRUE)

cn = sf::st_read("osm_os/cn.geojson")
geometry_type = sf::st_geometry_type(cn)
print(unique(geometry_type))
```