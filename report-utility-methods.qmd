...
distance_frequency_beta_1 = dd_data |>
  mutate(m_beta_1 = exp(log_n + average_distance * beta_value_1)) 
# For all beta values:
distance_frequency_beta = map_dfr(
  beta_values, ~ dd_data |>
    mutate(m_beta = exp(log_n + average_distance * .x), beta = .x),
  .id = "n"
)
distance_frequency_beta_summary = distance_frequency_beta |>
  group_by(beta) |>
  summarise(
    `Mean distance (km)` = weighted.mean(average_distance, m_beta),
    `Proportion 1-2 km` = sum(m_beta[average_distance <= 2]),
    `Proportion 2-5 km` = sum(m_beta[average_distance > 2 & average_distance <= 5])
  )
...
scatter_dist_number_sample = distance_frequency_beta_sample |>
  ...
  ggplot() +
  geom_point(aes(x = average_distance, y = m_beta, color = beta)) +
  labs(
    x = "Trip distance (km)",
    y = "Proportion of trips"
  ) +
