...
breaks_dist_euclidean = breaks_dist / 
  1.3 # Assuming average circuity of 1.3
...
We will model the number of trips as a function of distance using exponential decay.

```{r}
dd_data = desire_line_summary_1km |>
  filter(`Average distance (km)` > 1) |>
  transmute(
    average_distance = `Average distance (km)`,
    number_of_trips = `Number of trips (all)`
  )
dd_data$log_n = log(dd_data$number_of_trips)
# With lm function:
...
scatter_dist_number = desire_line_summary_1km |>
  filter(`Average distance (km)` > 1) |>
  ggplot() +
  geom_point(aes(x = `Average distance (km)`, y = `Number of trips (all)`), color = "blue") +
  labs(
    x = "Trip distance band",
    y = "Number of trips"
  ) +
