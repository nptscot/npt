...
group_ids = sapply(sf::st_geometry(network_c), function(geometry, index, lines) {
...
return(unioned)
}, index = lines_index, lines = network_c)
...
if (!inherits(connection_lines, "sf")) {
  ...
  connection_lines_sf <- st_sfc(connection_lines, crs = st_crs(cycle_net_filtered_i))
} else {
  connection_lines_sf <- connection_lines
}
if (st_crs(cycle_net_filtered_i) != st_crs(connection_lines_sf)) {
...
all_geometries_sfc <- st_sfc(all_geometries, crs = st_crs(cycle_net_filtered_i))


final_network <- st_sf(geometry = all_geometries_sfc)

mapview(final_network)
```
