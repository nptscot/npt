...
This shows that, for calculating the number of people arriving at each zone, we need a national dataset of zones: zone level statistics should not be calculated per region, for the number of people arriving at each zone at least.

In any case, we'll replace 'startDZ' and 'endDZ' with 'geo_code1' and 'geo_code2'.

This shows that there are 344 zones in the case study area, and the od data is at the zone level.
...
The input data is `r_utility_fastest`, let's first check that it contains the same number of trips as the `od_utility_combined` data.
...
The results show differences between the two datasets.
The differences are not big enough to explain the differences between the baseline and Go Dutch scenarios.
...
Let's check the total amount of trips for the Go Dutch scenario again, for each OD pair:
...
Let's check that the output is internally consistent, with the total of all modes equal to the total number of trips, for each scenario.
...
sum(routes_df$all)
mean(routes_df$all)
mean(r_utility_fastest_df$all)
table(routes_df$purpose)
table(r_utility_fastest_df$purpose)
```

This shows that we're losing a load of trips in the `get_uptake_scenarios` function.

A quick fix is to update the baseline stats object, replacing `od_utility_combined` with `uptake_utility_fastest`.
...
Let's re-run the `utility_stats` target and check the number of shopping trips in the Go Dutch scenario.
