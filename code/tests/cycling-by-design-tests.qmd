...
# let's calculate the reduction in nrows
nrow(osm_highways) / nrow(osm_lines)
...
table(osm_cbd$highway)
    #    busway      crossing      cycleway living_street       primary  primary_link   residential 
    #         8             1          4733            12           609            11           820 
    # secondary       service      tertiary tertiary_link         trunk    trunk_link  unclassified 
    #       358          1446           578             6           606            51           782 

# Todo: use table instead of unique
...
nrow(separate) # only 2 tagged as separate
...
nrow(segway) # 6 segregated
tm_shape(segway) + tm_lines()

osm_study = osm_cbd[study_area, ]

# These are off-road paths shared with pedestrians that have been tagged as such
segno = osm_study |> 
