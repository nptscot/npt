...
#| eval: false
file.copy("code/tests/segregation_levels.html", "segregation_levels.html", T)
system("gh release upload v1 segregation_levels.html")
```


## Results for Edinburgh

We can generate the results for Edinburgh:
...
edinburgh_3km = edinburgh |> 
  filter(circle_id <= 2)
cycle_network = get_cycling_network("Scotland")
driving_network = get_driving_network("Scotland")
edinburgh_cycle = cycle_network[edinburgh, , op = sf::st_within]
edinburgh_driving = driving_network[edinburgh, , op = sf::st_within]
...
# Upload the results:
file.copy("code/tests/segregation_levels_edinburgh.html", "segregation_levels_edinburgh.html", T)
system("gh release upload v1 segregation_levels_edinburgh.html")
```