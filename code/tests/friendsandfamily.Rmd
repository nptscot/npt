...
source(here::here("R/gravity_model.R"))
```
...
zones = readRDS(here::here("../inputdata/DataZones_population.Rds"))
...
trip_purposes = read.csv(here::here("./data-raw/scottish-household-survey-2012-19.csv"))
...
To estimate the number of trips for visiting friends and family, we use the resident population of each DataZone 
...
summary(od_interaction$distance_euclidean)
hist(od_interaction$distance_euclidean)
# Sample weighted towards OD pairs with high interaction

od_interaction_sampled = od_interaction %>%
  filter(quantile(interaction, 0.9) < interaction)

# Keep OD pairs with length below a threshold
od_interaction_threshold = od_interaction %>% 
  filter(distance_euclidean > min_distance_meters)

max_social_od_pairs = 10000 # could be 3000 k for national coverage
od_interaction_sampled = od_interaction_threshold %>% 
  sample_n(max_social_od_pairs)
  # sample_n(max_social_od_pairs, weight = interaction)

summary(sf::st_length(od_interaction_sampled))

tm_shape(od_interaction_sampled  %>% sample_n(1000)) + tm_lines()

# Proportion of visiting trips that we assume are being included (currently less than 2.7km)
prop_trips_included = 0.2

# Scale the interaction down to the actual number of projected visiting trips 
total_interaction = sum(od_interaction$interaction)
unique_origin = od_interaction %>% 
  select(origin_visiting_trips) %>% 
  st_drop_geometry()

zone_social_departing = od_interaction %>%
  sf::st_drop_geometry() %>%
  group_by(O) %>% 
  summarise(interaction_o = interaction)
unique_origin = unique(unique_origin)
origin_trips = sum(unique_origin$origin_visiting_trips)
origin_trips_included = origin_trips * prop_trips_included

od_interaction_scaled = od_interaction %>% 
  mutate(social_trips_estimated = interaction / total_interaction * origin_trips_included)


sum(od_interaction_scaled$interaction)


# od_interaction = od_interaction %>% 
#   mutate(interaction = interaction/50000)

# od_interaction_explore = od_interaction_scaled %>%
#   mutate(origin_density = origin_visiting_trips / origin_ResPop2011,
#          dest_density = destination_visiting_trips / destination_ResPop2011,
#          density_multiplied = origin_visiting_trips * destination_visiting_trips,
#          visits_multiplied = origin_ResPop2011 * destination_ResPop2011,
#          scaling = interaction / density_multiplied)
summary(od_interaction_explore)

# # Jittering
oa_subpoints = readRDS(here::here("../inputdata/oas.Rds"))
...
summary(od_interaction_scaled$interaction_scaled)
mean(od_interaction_scaled$interaction_scaled) / disag_threshold
# total_social_trips = 10000000
# multiplier = total_social_trips / sum(od_interaction_scaled$interaction_scaled)
# od_interaction_scaled = od_interaction_scaled %>% 
#   mutate(social_trips_estimated = interaction * multiplier)
sum(od_interaction_scaled$social_trips_estimated)
summary(od_interaction_scaled$social_trips_estimated)

od_interaction_jittered = odjitter::jitter(
  od = od_interaction_scaled,
...
#     length_euclidean_jittered = units::drop_units(st_length(od_interaction_jittered))/1000
#   ) %>%
#   filter(
...
# n_short_lines_removed = nrow(od_interaction_jittered) - nrow(od_visiting_jittered_updated)
