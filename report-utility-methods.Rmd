...
The number of daily trips for each purpose is available in the trip purpose percentage breakdown from the Scottish Household Survey, Table TD3 of Transport and Travel in Scotland 2019 travel diary tables (after adjusting these percentages to remove the 'Go Home' category).
That table shows 25.1% of trips being assigned for shopping, 11.7% for social, and 6.3% for leisure, compared with 23.3% for commuting (https://www.transport.gov.scot/media/51346/transport-and-travel-in-scotland-2019-travel-diary-tables.xlsx).
...
We took data from the National Travel Survey
...
nts0403_length = pivot_longer(nts0403_length_raw, -1) |>
  # Remove the space and everything after it:
    mutate(Year = str_remove(Year, " .+")) 
nts0403_length_subset = nts0403_length |>
  filter(str_detect(name, "Commute|Shop|Visit|Entertainment")) 
```
