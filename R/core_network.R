#' Function To generate core network
#' @param os_la os_la
#' @param osm_la osm_la
#' @param combined_network_tile combined_network_tile
#' @param la_name la_name
#' @param parameters parameters
#' @param lads lads
#' 
core_network = function(os_la, osm_la, combined_network_tile, lads, parameters, la_name) {
    library(sfnetworks)
    library(sf)
    library(tidygraph)
    library(tidyverse)
    ...
    combined_network_tile = combined_network_tile |>
        sf::st_transform(crs = "EPSG:27700")
    os_la$geometry = os_la$geom
    osm_la$geometry = osm_la$geom
    ...
    tryCatch(
        {
        la_name_boundary = lads |> filter(LAD23NM == la_name) |>
            sf::st_transform(crs = "EPSG:27700")
        ...
        if (!is.null(orcp_la_name_boundary)) {
            orcp_la_name_boundary_centroid = sf::st_centroid(sf::st_union(orcp_la_name_boundary))
            distance_orcp_to_cn = sf::st_distance(cohesive_network_la_name_boundary_centroid, orcp_la_name_boundary_centroid)
            } else {
            orcp_la_name_boundary_centroid = NULL
            distance_orcp_to_cn = units::set_units(100000, "m")
        }

        if (!is.null(orcp_la_name_boundary) && nrow(orcp_la_name_boundary) > 0 && (distance_orcp_to_cn <= units::set_units(5000, "m"))) {

            osm_la = osm_la[!is.na(osm_la$highway), ]

            orcp_la_name_boundary = find_orcp_path(orcp_la_name_boundary, cohesive_network_la_name_boundary, osm_la, os_la, combined_net_la_name_boundary)
